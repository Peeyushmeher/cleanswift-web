-- ============================================================================
-- Fix RPC Role Lookup Returning 'postgres' Instead of 'detailer'
-- ============================================================================
-- The RPC functions are incorrectly getting 'postgres' as the role instead
-- of the actual role from the profiles table. This migration fixes the role
-- lookup to be more explicit and ensure it correctly retrieves the role.
-- ============================================================================

-- ============================================================================
-- Update Function: set_detailer_availability
-- ============================================================================
-- Fix the role lookup query to explicitly check for row existence and
-- properly cast the role enum value
-- ============================================================================
...
BEGIN
  ...
  SELECT role, true INTO current_role, profile_exists
  FROM profiles
  WHERE id = current_user_id;

  IF NOT profile_exists OR current_role IS NULL THEN
  ...
  IF current_role::text != 'detailer' THEN
    RAISE EXCEPTION 'Only detailers can set availability. Current role: %, user_id: %', current_role::text, current_user_id;
  END IF;
...
BEGIN
  ...
  IF p_detailer_id IS NOT NULL THEN
    ...
    IF current_role::text = 'admin' THEN
      target_detailer_id := p_detailer_id;
    ELSIF current_role::text = 'detailer' THEN
      ...
      END IF;
    ELSE
      RAISE EXCEPTION 'Only detailers and admins can view days off. Current role: %, user_id: %', current_role::text, current_user_id;
    END IF;
  ELSE
    -- No detailer_id provided, use current user's detailer record
    IF current_role::text != 'detailer' THEN
      RAISE EXCEPTION 'Detailer profile not found for this user. Current role: %, user_id: %', current_role::text, current_user_id;
    END IF;
...
-- ============================================================================
-- Also fix the role lookup in get_detailer_availability for consistency
-- ============================================================================

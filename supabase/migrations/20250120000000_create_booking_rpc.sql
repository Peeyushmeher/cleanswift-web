...
-- ============================================================================
-- This migration creates a secure RPC function to create bookings transactionally.
-- The function:
-- 1. Uses auth.uid() internally (does not accept user_id as parameter)
-- 2. Validates car ownership
-- 3. Validates services are active
-- 4. Inserts into bookings and booking_services in a single transaction
-- 5. Returns booking data with computed totals
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO bookings (
    ...
    status,  -- Old column: use 'scheduled' (valid values: scheduled, in_progress, completed, canceled)
  ...
  VALUES (
    ...
    p_location_lng,
    p_location_notes,
    'requires_payment',  -- Booking created, waiting for payment
    'requires_payment',  -- New payment_status
...
COMMENT ON FUNCTION public.create_booking IS 'Creates a booking transactionally. Validates car ownership and service validity. Returns booking data with computed totals. Requires city, province, and postal_code as separate parameters.';
...
-- Migration Complete
-- ============================================================================
-- The create_booking RPC function is now available for use by authenticated users.
-- It enforces:
-- - Authentication (uses auth.uid() internally)
-- - Car ownership validation
-- - Service validity and active status
-- - Transactional booking creation (bookings + booking_services)
-- ============================================================================

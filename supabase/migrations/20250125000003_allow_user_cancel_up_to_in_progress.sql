-- ============================================================================
-- Allow Users to Cancel Bookings Up to In Progress Status
-- ============================================================================
-- This migration updates the update_booking_status function to allow users
-- to cancel bookings all the way up to when the job is in progress.
-- Users can now cancel: pending, requires_payment, paid, offered, accepted, in_progress
-- Users CANNOT cancel: completed, cancelled, no_show (final states)
-- ============================================================================
...
BEGIN
  ...
  IF current_role = 'user' THEN
    ...
    IF p_new_status = 'cancelled' THEN
      IF current_status IN ('pending', 'requires_payment', 'paid', 'offered', 'accepted', 'in_progress') THEN
        allowed := true;
      END IF;
    END IF;
...
'Central booking status state machine. Enforces allowed state transitions based on user role and current status. Uses row locking to prevent race conditions. Users can now cancel bookings up to in_progress status (pending, requires_payment, paid, offered, accepted, in_progress).';

-- ============================================================================
-- Migration Complete
-- ============================================================================
-- This migration allows users to cancel bookings all the way up to when
-- the job is in progress. Users can cancel: pending, requires_payment, paid,
-- offered, accepted, and in_progress. Users cannot cancel completed, cancelled,
-- or no_show bookings (final states).
-- ============================================================================

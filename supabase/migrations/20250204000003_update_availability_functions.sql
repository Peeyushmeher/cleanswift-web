-- ============================================================================
-- Update Availability Management Functions
-- ============================================================================
-- This migration updates availability management functions to support:
-- 1. Lunch breaks in set_detailer_availability
-- 2. Lunch breaks in get_detailer_availability return value
-- 3. Days off management functions (add, remove, get)
-- ============================================================================

-- ============================================================================
-- Update Function: set_detailer_availability
-- ============================================================================
-- Adds optional lunch break parameters to set_detailer_availability
-- ============================================================================
...
-- ============================================================================
-- Updates to return lunch break times in the JSON response
-- ============================================================================
...
-- ============================================================================
-- Function: add_detailer_day_off
-- ============================================================================
-- Adds a day off for a detailer
--
-- Parameters:
--   p_date - The date to mark as a day off
--   p_reason - Optional reason for the day off
--
-- Returns: The created day off record
-- ============================================================================
...
-- ============================================================================
-- Function: remove_detailer_day_off
-- ============================================================================
-- Removes (deactivates) a day off for a detailer
--
-- Parameters:
--   p_date - The date to remove from days off
--
-- Returns: The updated day off record
-- ============================================================================
...
-- ============================================================================
-- Function: get_detailer_days_off
-- ============================================================================
-- Gets days off for a detailer
--
-- Parameters:
--   p_detailer_id - Optional detailer ID (defaults to current user's detailer)
--   p_start_date - Optional start date filter (defaults to today)
--   p_end_date - Optional end date filter (defaults to 1 year from start)
--
...
BEGIN
  ...
  IF p_detailer_id IS NOT NULL THEN
    ...
    ELSIF current_role = 'detailer' THEN
      ...
      IF target_detailer_id IS NULL THEN
        RAISE EXCEPTION 'Cannot access another detailer''s days off';
      END IF;
    ELSE
      RAISE EXCEPTION 'Only detailers and admins can view days off';
    END IF;
  ELSE
    -- No detailer_id provided, use current user's detailer record
    IF current_role != 'detailer' THEN
...
GRANT EXECUTE ON FUNCTION public.add_detailer_day_off TO authenticated;
GRANT EXECUTE ON FUNCTION public.remove_detailer_day_off TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_detailer_days_off TO authenticated;

-- ============================================================================
-- Migration Complete
-- ============================================================================
-- Availability management functions are now updated:
-- 1. set_detailer_availability - Now supports lunch breaks
-- 2. get_detailer_availability - Now returns lunch breaks
-- 3. add_detailer_day_off - Add a day off
-- 4. remove_detailer_day_off - Remove a day off
-- 5. get_detailer_days_off - Get days off for a detailer
-- ============================================================================

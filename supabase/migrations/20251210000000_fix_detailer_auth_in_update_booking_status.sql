-- ============================================================================
-- Fix Detailer Authentication Check in update_booking_status Function
-- ============================================================================
-- This migration fixes the detailer authentication check in update_booking_status.
-- The previous version incorrectly compared detailer.id to profile.id (auth.uid()).
-- The fix checks if the detailer's profile_id matches the current user's auth.uid().
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_role = 'detailer' THEN
    -- FIX: Check if booking's detailer_id matches a detailer record linked to current user
    -- Instead of comparing detailer.id to profile.id, we check if detailer.profile_id = auth.uid()
    IF v_detailer_id IS NULL OR NOT EXISTS (
      SELECT 1 FROM detailers d 
      WHERE d.id = v_detailer_id 
      AND d.profile_id = v_user_id
    ) THEN
  ...
  IF p_new_status = 'completed' AND v_detailer_id IS NOT NULL THEN
    ...
    IF v_detailer_org_id IS NULL AND v_detailer_stripe_account_id IS NOT NULL THEN
      ...
      IF NOT EXISTS (
        ...
        v_platform_fee := (v_service_price * v_platform_fee_percentage) / 100;
        v_payout_amount := v_service_price - v_platform_fee;
...
'Central booking status state machine. Enforces allowed state transitions based on user role and current status. Uses row locking to prevent race conditions. Automatically initiates Stripe Connect transfers for solo detailers when bookings are completed. Fixed detailer authentication check to compare detailer.profile_id with auth.uid() instead of detailer.id.';

-- ============================================================================
-- Migration Complete
-- ============================================================================
-- The update_booking_status function now correctly checks if a detailer is
-- assigned to a booking by verifying that the detailer's profile_id matches
-- the current user's auth.uid(), rather than incorrectly comparing detailer.id
-- to profile.id.
-- ============================================================================

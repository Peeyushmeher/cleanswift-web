-- ============================================================================
-- Fix Availability Unique Constraint Issue
-- ============================================================================
-- The set_detailer_availability function was looking up existing availability
-- by (detailer_id, day_of_week, start_time, end_time). This caused issues when
-- trying to toggle an existing day's availability because the client might
-- pass different times than what's stored.
--
-- Fix: Look up existing availability by just (detailer_id, day_of_week) and
-- update all fields including times.
-- ============================================================================
...
BEGIN
  ...
  -- 8) Check if availability already exists for this day (regardless of times)
  -- FIXED: Only check by day_of_week, not by times
  ...
  IF existing_availability IS NOT NULL THEN
    -- Update existing record including times
...
'Sets availability slot for a detailer with optional lunch break. If a slot already exists for the day, it updates that slot. Detailer only.';
